...
data = pd.read_csv('D:\\Others\\Demo-Works\\Crypto-Chatbot-master\\Backend\\combined_data.csv')
...
def compareit(name1, name2):
    ...
    # Check if all values are positive, else fallback to linear scale
    if (cryp1['Close'] > 0).all() and (cryp2['Close'] > 0).all():
        plt.yscale('log')
    else:
        plt.yscale('linear')
